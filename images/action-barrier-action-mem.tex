...
\node [pipestageinner-mem,above=of vs.south,yshift=-2mm] (vs-mem) {WRITE $img_1$};
...
\node [pipestageinner-mem,above=of fs.south,yshift=-2mm] (fs-mem) {READ $img_1$};
\node [pipestageinner,right=of fs] (ca) {COLR};
\node [pipestageinner,right=of ca] (comp) {COMP};
\node [pipestageinner,right=of comp] (xfer) {XFER};
%
...
%
\node [pipestagemem,node distance=12mm,below=of src-bottom] (flush1) {FLUSH, $img_1$, \texttt{VERT|XFER}, \texttt{SHADER\_WRITE}};
